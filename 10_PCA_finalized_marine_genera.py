...
Fish = ["Benthosema", "Decapterus", "Caesio", "Engraulis", "Plotosus"]
...
Fungi = ["Sporisorium", "Annulohypoxylon", "Paraphaeosphaeria", "Hypomontagnella"]


GROUPS_TO_COLOUR = {
    "Cnidaria":       (Cnidaria,       "#C4685D"),
    "Green_algae":    (Green_algae,    "#07FE38"),
    "Sharks":         (Sharks,         "#DE33D3"),
    "Ascidian":       (Ascidian,       "#C4E50B"),
    "Fish":           (Fish,           "#206CDF"),
    "Sponges":        (Sponges,        "#E0AB0D"),
    "Mollusca":       (Mollusca,       "#D10BF0"),
    "Diatomes":       (Diatomes,       "#08CAE3"),
    "Marine_plants":  (Marine_plants,  "#4EF60C"),
...
###############################################
# And just to check, I made the script write how many genera from the whitelist
# it was working with - that way I could ensure that they were all found and included
# in the PCA analysis and biplots.         
###############################################

GENUS_WHITELIST = load_genus_whitelist(GENUS_LIST_EXCEL)
print(f"Including {len(GENUS_WHITELIST)} genera from Excel whitelist.")

###############################################
# Here deifine a function to extract the CGG IDs
###############################################
...
for sample_id, dtype in sorted(all_keys):
    key = (sample_id, dtype)
    row = {"sample_id": sample_id, "Data_type": dtype, "age_bp": ss_ages.get(key, ds_ages.get(key)), "total_reads": ss_totals.get(key, ds_totals.get(key))}
    for t in all_taxa:
...
###############################################
# All genera that are not in the whitelist are excluded by the following part
###############################################
...
missing = sorted(GENUS_WHITELIST - set(taxa_cols))

raw = raw[list(meta_cols) + keep_taxa]

###############################################
# And using the MIN_TOTAL_READS, i remove all taxa with less than the (here 50)
# n_reads defined at the top of the script. These raw counts are saved as a .csv
# for a manual check  
###############################################
...
excluding_genera = [g for g in GENERA_TO_EXCLUDE if g in raw.columns]
if excluding_genera:
    print(f"Excluding genera: {excluding_genera}")
    raw = raw.drop(columns=excluding_genera)
taxa_cols = [c for c in raw.columns if c not in meta_cols]
raw.to_csv(
    os.path.join(OUTDIR, "filtered_raw_counts_whitelist.csv"),
    index=False
)
...
rpm.to_csv(
    os.path.join(OUTDIR, "rpm_normalized_to_LCA_rowcounts.csv"),
    index=False
)
...
###############################################
# Here I define a function to make tables of the PCA-loading for the genera on PC1
# and PC2  
###############################################

def make_driver_table(loadings: pd.DataFrame, X_df: pd.DataFrame, scores_df: pd.DataFrame) -> pd.DataFrame:
    drivers = loadings.copy()

    drivers["abs_PC1"] = drivers["PC1"].abs()
    drivers["abs_PC2"] = drivers["PC2"].abs()
    drivers["primary_axis"] = np.where(drivers["abs_PC1"] >= drivers["abs_PC2"], "PC1", "PC2")

    pc1 = scores_df["PC1"].values
    pc2 = scores_df["PC2"].values
    drivers["corr_with_PC1"] = [np.corrcoef(X_df[g].values, pc1)[0, 1] for g in drivers.index]
    drivers["corr_with_PC2"] = [np.corrcoef(X_df[g].values, pc2)[0, 1] for g in drivers.index]

    drivers["max_abs_loading"] = np.maximum(drivers["abs_PC1"], drivers["abs_PC2"])
    return drivers.sort_values("max_abs_loading", ascending=False)

###############################################
# This function runs the PCA and return scores, loading and a model that contain the
# eigenvectors, eigenvalues and the explained variance.  
###############################################


def run_pca_and_save_drivers(X_df: pd.DataFrame, meta_df: pd.DataFrame, prefix: str, outdir: str):
    scores_df, loadings_df, pca_model = do_pca_with_loadings(X_df, n_components=2)

    save_pca_eigenvalues(pca_model, prefix, outdir)

    drivers_df = make_driver_table(loadings_df, X_df, scores_df)
    drivers_df.to_csv(os.path.join(outdir, f"PCA_driver_genera_{prefix}.csv"))

    scores_meta = meta_df.join(scores_df)
    return scores_meta, drivers_df, pca_model, loadings_df

###############################################
#  Here the I run the PCA based on the data transformation above (Hellinger)
#  for both the SS and DS data as well as SS and DS together.   
###############################################

meta = rpm[["sample_id", "Data_type", "age_bp"]].copy()
meta.index = rpm.index

df_hell_pooled, hell_drivers_pooled, pca_hell_pooled, load_hell_pooled = run_pca_and_save_drivers(
    hell_df, meta, "Hellinger_POOLED", OUTDIR
)
df_hell_ss, hell_drivers_ss, pca_hell_ss, load_hell_ss = run_pca_and_save_drivers(
    hell_df.loc[meta["Data_type"] == "SS"], meta.loc[meta["Data_type"] == "SS"], "Hellinger_SS", OUTDIR
)
df_hell_ds, hell_drivers_ds, pca_hell_ds, load_hell_ds = run_pca_and_save_drivers(
    hell_df.loc[meta["Data_type"] == "DS"], meta.loc[meta["Data_type"] == "DS"], "Hellinger_DS", OUTDIR
)
...
def autoscale_arrow_scale(scores_df: pd.DataFrame, loadings_df: pd.DataFrame, top_n: int = TOP_N_ARROWS, fallback: float = ARROW_SCALE_FALLBACK) -> float:
    score_radius = max(np.nanmax(np.abs(scores_df["PC1"].values)), np.nanmax(np.abs(scores_df["PC2"].values)))
...
def plot_pca_biplot_separate(df_scores_meta: pd.DataFrame, pca_model: PCA, loadings_df: pd.DataFrame, title: str, outpath: str, top_n_arrows: int = TOP_N_ARROWS, arrow_scale: Optional[float] = None):
    ...
    for taxon, row in top_load.iterrows():
        ...
        texts.append(
            ax.text(x * 1.08, y * 1.08, str(taxon),
                    fontsize=16, color=color, fontweight="bold", fontstyle="italic", zorder=4)
        )
