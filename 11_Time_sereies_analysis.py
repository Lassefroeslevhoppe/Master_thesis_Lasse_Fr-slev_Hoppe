...
def run_rshift_global(pca_scores_csv: str, out_global_csv: str):
    if shutil.which(RSCRIPT_BIN) is None:
        msg = f"[ERROR] {RSCRIPT_BIN} not found in PATH."
        if FAIL_IF_NO_RSCRIPT:
...
PCA_CSV <- "{esc(pca_scores_csv)}"
OUT_GLOBAL <- "{esc(out_global_csv)}"

L <- {int(STARS_L)}
P <- {float(STARS_P)}
min_n <- 2*L + 1

pca <- read_csv(PCA_CSV, show_col_types = FALSE) %>%
  mutate(age_ka = as.numeric(age_ka),
         PC1 = as.numeric(PC1),
         PC2 = as.numeric(PC2)) %>%
...
run_axis <- function(axis_col) {{
  ...
  sh <- Rodionov(d, col="y", time="age_ka", l=L, prob=P, merge=FALSE)
  sh <- as_tibble(sh)
  tibble(axis = axis_col,
         shift_age_ka = as.numeric(sh[[1]]),
         RSI = as.numeric(sh[[2]]))
}}
...
"""
    r_path = os.path.join(OUTDIR, "run_rshift_global.R")
    ...
    if cp.returncode != 0:
        print(cp.stdout)
        print(cp.stderr)
        raise RuntimeError("[ERROR] Rscript run failed.")
...
def plot_global_shift(
    shifts_ss: List[float],
    shifts_ds: List[float],
    ages_ss: np.ndarray,
    ages_ds: np.ndarray,
    out_prefix: str
):
    ...
    for ax, label, shifts, ages in [
        (axes[0], "SS", shifts_ss, ages_ss),
        (axes[1], "DS", shifts_ds, ages_ds),
    ]:
        shade_age_bands(ax)
    ...
    if INVERT_AGE_AXIS:
        ...
        axes[0].set_xlim(0.0, MAXIXIMUM_AGE)
...
run_rshift_global(pca_ss_csv, out_ss)
run_rshift_global(pca_ds_csv, out_ds)
